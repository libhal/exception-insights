...
void elf_parser_tests()
{
    ...
    "Elf_Parser"_test = [] {
        std::string_view test_file = "../../tests/testing_programs/demo_class";
        using namespace boost::ut;
        "Validate the Elf Header"_test = [test_file]() {
            ...
            for(int i = 0; i < 4; i++){
                ...
                "Expecting: " << elf_magic_number[i] << ". Got" << ehdr.e_ident[i] << "\n";
            }
        };
        ...
        "Validate the Program Headers"_test = [test_file]() {
            ...
            "Expect number of programs: " << ehdr.e_phnum << 
            ". Got" << phdr.size() << "\n";
        };

        "Validate the Section Headers by finding .gcc_expect_table"_test = [test_file]() {
            ElfParser elf(test_file);
            std::string_view section_name = ".gcc_except_table";
            auto shdr = elf.get_section(section_name);

            expect(shdr.has_value()) <<
            "Expect finding .gcc_except_table\n";
        };

        "Validate the Symbol Table"_test = [test_file]() {
            ElfParser elf(test_file);
            auto sym = elf.get_symbol_table();
            expect(sym.has_value()) << 
