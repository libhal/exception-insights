    ...
    "StateMachine"_test = [] {
        "Initalize inital state"_test = [] {
            ...
            expect(typeid(actual_state_ref) == typeid(UserInputState))
              << "Initial state should be UserInputState";
        };

        "Transition State"_test = [] {
            StateMachine sm;
            std::optional<std::unique_ptr<State>> mock_state
              = std::make_unique<MockState>();
            ...
            State& actual_state_ref = actual_state.value().get();
            expect(typeid(actual_state_ref) == typeid(MockState))
        ...
        "State Transition Order"_test = [] {
            ...
            std::vector<std::type_index> state_order
              = { typeid(UserInputState), typeid(ElfParserState),
                  typeid(CallgraphState), typeid(AbiParserState),
                  typeid(ValidatorState), typeid(OutputState) };
